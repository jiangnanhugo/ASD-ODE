...
#
def try_differeent_active_learning_strategies(grammar_expressions, grammar_model, task, num_init_conds, active_mode,
                                              full_mesh_size):
    num_repetitions = 5
    ...
    for i in range(num_repetitions):
        ...
        if active_mode == 'default':
            ...
            print("default time {} sec".format(np.round(end_time, 3)))
        ...
        if active_mode == 'coreset':
            ...
            top_pred_default = deep_coreset(true_traj, num_init_conds)
            # top_pred_default = grammar_model.expression_active_evaluation(temp, active_mode='default')
            default_pred_list.append(top_pred_default)
            #####
            end_time = time.time() - start
            ...
            print("coreset time {} sec".format(np.round(end_time, 3)))
            #####
        elif active_mode == 'qbc':
            start = time.time()
            temp = copy.deepcopy(grammar_expressions)
            top_pred = grammar_model.expression_active_evaluation(temp, active_mode='query_by_committee',
                                                                  given_region=region)
            ...
            print("{} time {} sec".format(active_mode, np.round(end_time, 3)))
...
@click.option('--num_init_conds', default=100, type=int, help="batch of initial condition of dataset")
@click.option('--num_regions', default=20, type=int, help="number of regions to be sampled")
@click.option('--region_width', default=0.2, type=float, help="")
...
@click.option('--memray_output_bin', default='output.bin', type=str, help="")
@click.option('--track_memory', default=False, help="")
def main(equation_name, pred_expressions_file, num_init_conds, num_regions, region_width, noise_type, noise_scale,
         active_mode, full_mesh_size, memray_output_bin, track_memory):
    ...
    if track_memory:
        ...
        with memray.Tracker(memray_output_bin):
            start = time.time()
            try_differeent_active_learning_strategies(grammar_expressions, grammar_model, task,
                                                      num_init_conds,
                                                      active_mode,
                                                      full_mesh_size)
            used = time.time()
        print("done in {} seconds".format(used))
