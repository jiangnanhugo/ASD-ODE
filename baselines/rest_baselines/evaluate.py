...
class Evaluator(object):
    ...
    def prepare_test_trajectory(
        ...
        if evaluation_task == "interpolation":
            ...
            for time, trajectory, tree in zip(samples["train"]["times"], samples["train"]["trajectories"],
                                              samples["tree"]):
                y0 = trajectory[-1]
                t0 = time[-1]
            ...
            for time, trajectory, tree, dimension in zip(samples["train"]["times"], samples["train"]["trajectories"],
                                                         samples["tree"], samples["infos"]["dimension"]):
    ...
    def _evaluate(
        ...
        zipped = [times, trajectories, trees,
                  (all_candidates.values() if isinstance(all_candidates, Dict) else all_candidates)]
        ...
        for items in zip(*zipped):
            ...
            if isinstance(best_candidate, str) and (
                    not hasattr(self.params, "convert_prediction_to_tree") or self.params.convert_prediction_to_tree):
                try:
    ...
    def evaluate_on_iterator(self, iterator, name="in_domain"):
        ...
        for samples_i, samples in enumerate(tqdm(iterator, total=_total)):
            ...
            best_results, best_candidates = self._evaluate(
                original_times, original_trajectories, trees, all_candidates, all_duration_fit,
                self.params.validation_metrics
            )
            ...
            batch_results["predicted_trees"].extend(
                [tree.infix() if hasattr(tree, 'infix') else tree for tree in best_candidates])
            for k, v in infos.items():
    ...
    def evaluate_on_pmlb(
        ...
        if path_dataset is not None and os.path.exists(path_dataset):
            ...
            strogatz_equations = {
                ...
                "strogatz_shearflow1": '(cos(u(2))/sin(u(2)))*cos(u(1)) | (cos(u(2))^2+0.1*sin(u(2))^2)*sin(u(1))',
                # replaced cot(x) with cos(x) / sin(x)
...
if __name__ == "__main__":
    ...
    if params.reload_checkpoint:
        ...
        for p in params.__dict__:
            if p in pickled_args and p not in ["eval_dump_path", "dump_path", "reload_checkpoint", "rescale",
                                               "validation_metrics", "eval_in_domain", "eval_on_pmlb",
                                               "batch_size_eval", "beam_size", "beam_selection_metric",
                                               "subsample_prob", "eval_noise_gamma", "eval_subsample_ratio",
                                               "use_wandb", "eval_size", "reload_data"]:
