...
class SymbolicTransformerRegressor(BaseEstimator, PredictionIntegrationMixin):
    ...
    def __init__(self,
        ...
        self.scaler = utils_wrapper.Scaler(time_range=[1, time_range],
                                           feature_scale=feature_scale) if self.rescale else None
    ...
    def fit(
        ...
        assert not (
                    self.model.average_trajectories and self.rescale), "Cannot average trajectories and rescale at the same time"
        ...
        if sort_candidates:
            for input_id in all_candidates.keys():
                all_candidates[input_id] = self.sort_candidates(times[input_id], trajectories[input_id],
                                                                all_candidates[input_id], metric=sort_metric,
                                                                verbose=verbose)
