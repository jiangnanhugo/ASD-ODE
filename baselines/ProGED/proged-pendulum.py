...
data = generate_ODE_data(system='pendulum_on_cart', inits=np.random.randn(nvars),
                         **generation_settings)
data = pd.DataFrame(data, columns=['t', 'x0', 'x1', 'x2', "x3"])

ED = EqDisco(data=data,
             task_type="differential",
             lhs_vars=["d_x0", "d_x1", "d_x2", "d_x3"],
             system_size=4,
             rhs_vars=["x0", "x1", "x2", "x3"],
             generator="grammar",
...
models = ModelBox()
for mi in ED.models:
    models.add_model([str(xi) for xi in mi.expr], symbols={"x": ["x0", "x1", "x2", "x3"], "const": "C"})
