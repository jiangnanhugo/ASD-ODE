...
--time_sequence_drop_rate $irregular_rate \
		--metric_name $metric_name --num_init_conds $num_init_conds --noise_type $noise_type --noise_scale $noise_scale >$dump_dir/${eq_name}.noise_${noise_type}${noise_scale}.irre$irregular_rate.$method.out
EOT
done
